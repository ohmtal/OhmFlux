...
//XXTH #define ICON_SIZE ImGui::GetFont()->FontSize + 3
...
namespace ifd {
	...
	bool PathBox(const char* label, std::filesystem::path& path, char* pathBuffer, ImVec2 size_arg) {
		...
		if (!(*state & 0b001)) {
			...
			for (auto comp : path) {
				std::string section = to_native(comp.u8string());
			...
			for (size_t i = 0; i < btnList.size(); i++) {
				...
				if (!isFirstElement) {
					...
					//XXTH anyOtherHC |= ImGui::IsItemHovered() | ImGui::IsItemClicked();
				...
				if (ImGui::Button(btnList[i].c_str(), ImVec2(0, GUI_ELEMENT_SIZE))) {
					...
					//XXTH path = std::filesystem::u8path(newPath);
					path = std::filesystem::path(reinterpret_cast<const char8_t*>(newPath.data()));
			...
			if (!anyOtherHC && clicked) {
				strcpy(pathBuffer, to_native(path.u8string()).c_str());
				*state |= 0b001;
		...
		// input box
			...
			if (ImGui::InputTextEx("##pathbox_input", "", pathBuffer, 1024, size_arg, ImGuiInputTextFlags_EnterReturnsTrue)) {
				...
				if (std::filesystem::exists(tempStr)) {
					path = std::filesystem::path(reinterpret_cast<const char8_t*>(tempStr.data()));
					//XXTH path = std::filesystem::u8path(tempStr);
				}
				ret = true;
			}
	...
	FileDialog::FileData::FileData(const std::filesystem::path& path) {
		...
		struct stat attr;
		stat(to_native(path.u8string()).c_str(), &attr);
	...
	FileDialog::FileDialog() {
		...
		for (const auto& entry : std::filesystem::directory_iterator("/", ec)) {
			if (std::filesystem::is_directory(entry, ec))
				thisPC->Children.push_back(new FileTreeNode(to_native(entry.path().u8string())));
		}
		m_treeCache.push_back(thisPC);
#endif
	}
	...
	bool FileDialog::Save(const std::string& key, const std::string& title, const std::string& filter, const std::string& startingDir)
	{
		...
		if (!startingDir.empty()) {
			//XXTH m_setDirectory(std::filesystem::u8path(startingDir), false);
			m_setDirectory(std::filesystem::path(reinterpret_cast<const char8_t*>(startingDir.data())), false);
	...
	{
		auto itr = std::find(m_favorites.begin(), m_favorites.end(), to_native(m_currentDirectory.u8string()));
	...
	void FileDialog::AddFavorite(const std::string& path)
	{
		...
		// XXTH  if (!std::filesystem::exists(std::filesystem::u8path(path)))
		if (!std::filesystem::exists(std::filesystem::path(reinterpret_cast<const char8_t*>(path.data()))))
		 	return;
	...
	void FileDialog::m_select(const std::filesystem::path& path, bool isCtrlDown)
	{
		...
		if (m_selections.size() == 1) {
			std::string filename = to_native( m_selections[0].filename().u8string());
			if (filename.size() == 0)
				filename = to_native(m_selections[0].u8string()); // drive
			...
			for (const auto& sel : m_selections) {
				std::string filename = to_native( sel.filename().u8string());
				if (filename.size() == 0)
					filename = to_native(sel.u8string());
	...
	bool FileDialog::m_finalize(const std::string& filename)
	{
		...
		if (hasResult) {
			if (!m_isMultiselect || m_selections.size() <= 1) {
				//XXTH std::filesystem::path path = std::filesystem::u8path(filename);
				std::filesystem::path path(reinterpret_cast<const char8_t*>(filename.data()));
	...
	void* FileDialog::m_getIcon(const std::filesystem::path& path)
	{
		...
		return m_icons[pathU8];
#else
		if (m_icons.count(to_native(path.u8string())) > 0)
			return m_icons[to_native(path.u8string())];

		std::string pathU8 = to_native(path.u8string());
	...
	void FileDialog::m_loadPreview()
	{
		for (size_t i = 0; m_previewLoaderRunning && i < m_content.size(); i++) {
			...
			if (data.Path.has_extension()) {
				std::string ext = to_native(data.Path.extension().u8string());
				if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".bmp" || ext == ".tga") {
					int width, height, nrChannels;
					unsigned char* image = stbi_load(to_native(data.Path.u8string()).c_str(), &width, &height, &nrChannels, STBI_rgb_alpha);
	...
	void FileDialog::m_setDirectory(const std::filesystem::path& p, bool addHistory)
	{
		...
		if (u8_str.size() == 2 && u8_str[1] == ':') {
			auto drive = u8_str + u8"\\";
			m_currentDirectory = std::filesystem::path(drive);
		}
		...
		// XXTH if (p.u8string() == "Quick Access") {
		if (to_native(p.u8string()) == "Quick Access") {
		...
		// XXTH else if (p.u8string() == "This PC") {
		else if (to_native(p.u8string()) == "This PC") {
			for (auto& node : m_treeCache) {
				if (node->Path == p)
					for (auto& c : node->Children)
						m_content.push_back(FileData(c->Path));
			...
			if (std::filesystem::exists(m_currentDirectory, ec))
				for (const auto& entry : std::filesystem::directory_iterator(m_currentDirectory, ec)) {
					...
					if (m_searchBuffer[0]) {
						std::string filename = to_native(info.Path.u8string());
					...
					if (!info.IsDirectory && m_type != IFD_DIALOG_DIRECTORY) {
						if (m_filterSelection < m_filterExtensions.size()) {
							...
							if (exts.size() > 0) {
								std::string extension = to_native(info.Path.extension().u8string());
	...
	void FileDialog::m_sortContent(unsigned int column, unsigned int sortDirection)
	{
		...
		if (m_content.size() > 0) {
			...
			auto compareFn = [column, sortDirection](const FileData& left, const FileData& right) -> bool {
				// name
				if (column == 0) {
					std::string lName = to_native(left.Path.u8string());
					std::string rName = to_native(right.Path.u8string());
	...
	void FileDialog::m_renderTree(FileTreeNode* node)
	{
		...
		std::string displayName = to_native(node->Path.stem().u8string());
		if (displayName.size() == 0)
			displayName = to_native(node->Path.u8string());
			...
			if (!node->Read) {
				...
				if (std::filesystem::exists(node->Path, ec))
					for (const auto& entry : std::filesystem::directory_iterator(node->Path, ec)) {
						if (std::filesystem::is_directory(entry, ec))
							node->Children.push_back(new FileTreeNode(to_native(entry.path().u8string())));
					}
				node->Read = true;
			}
	...
	void FileDialog::m_renderContent()
	{
		...
		if (m_zoom == 1.0f) {
			if (ImGui::BeginTable("##contentTable", 3, /*ImGuiTableFlags_Resizable |*/ ImGuiTableFlags_Sortable, ImVec2(0, -FLT_MIN))) {
				...
				for (auto& entry : m_content) {
					std::string filename = to_native(entry.Path.filename().u8string());
					if (filename.size() == 0)
						filename = to_native(entry.Path.u8string()); // drive
		...
		// "icon" view
			...
			for (auto& entry : m_content) {
				...
				//XXTH if (FileIcon(filename.c_str(), isSelected, entry.HasIconPreview ? entry.IconPreview : (ImTextureID)m_getIcon(entry.Path), ImVec2(32 + 16 * m_zoom, 32 + 16 * m_zoom), entry.HasIconPreview, entry.IconPreviewWidth, entry.IconPreviewHeight)) {
				// Falls m_getIcon(std::string) erwartet:
				if (FileIcon(filename.c_str()
					, isSelected
					, (ImTextureID)(entry.HasIconPreview ? (ImTextureID)entry.IconPreview : (ImTextureID)m_getIcon(entry.Path))
					, ImVec2(32 + 16 * m_zoom, 32 + 16 * m_zoom)
					, entry.HasIconPreview, entry.IconPreviewWidth, entry.IconPreviewHeight))
				{
	...
	void FileDialog::m_renderPopups()
	{
		...
		if (ImGui::BeginPopupModal("Are you sure?##delete")) {
			if (m_selectedFileItem >= static_cast<int>(m_content.size()) || m_content.size() == 0)
				...
				ImGui::TextWrapped("Are you sure you want to delete %s?", to_native(data.Path.filename().u8string()).c_str());
				if (ImGui::Button("Yes")) {
					std::error_code ec;
	...
	void FileDialog::m_renderFileDialog()
	{
		...
		if (FavoriteButton("##dirfav", std::count(m_favorites.begin(), m_favorites.end(), m_currentDirectory.u8string()))) {
			if (std::count(m_favorites.begin(), m_favorites.end(), m_currentDirectory.u8string()))
				RemoveFavorite(to_native(m_currentDirectory.u8string()));
			else 
				AddFavorite(to_native(m_currentDirectory.u8string()));
		}
		ImGui::SameLine();
		ImGui::PopStyleColor();
